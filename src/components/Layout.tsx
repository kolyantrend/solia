...
import { Home, ImagePlus, Trophy, User, Globe, Sun, Moon, BarChart3, Wallet, LogOut } from 'lucide-react';
...
export const Layout: FC<LayoutProps> = ({ children, activeTab, setActiveTab }) => {
  ...
  const handleDisconnect = useCallback(async () => {
    ...
    // Clear all cached wallet state so next connect picks fresh wallet
    ...
    <div className="flex flex-col h-[100dvh] bg-zinc-950 text-zinc-50 font-sans">
      ...
      <div className="flex items-center justify-between px-2 py-2.5 sm:p-4 max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-0.5 sm:gap-2">
          ...
          {/* Telegram link */}
          <a
            href="https://t.me/+1kvc0wLDvVg4NWVi"
            target="_blank"
            rel="noreferrer"
            className="p-1 sm:p-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-300 transition-colors"
            title="Telegram"
      ...
      {/* Bottom Navigation */}
      <nav className="fixed bottom-0 left-0 right-0 z-[90] bg-zinc-950/90 backdrop-blur-xl border-t border-zinc-800/50" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
