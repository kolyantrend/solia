      ...
      {followers.length > 0 && (
        <div className="bg-zinc-900/50 p-4 rounded-2xl border border-zinc-800/50">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-semibold text-zinc-300">{t('prof.followers')}</h3>
          </div>
          <div className="flex -space-x-2 overflow-x-auto hide-scrollbar">
            {followers.slice(0, 15).map((addr) => (
              <button
                key={addr}
                onClick={() => onViewProfile?.(addr)}
                className="shrink-0 hover:scale-110 transition-transform border-2 border-zinc-900 rounded-full"
